...
class FollowPagingSource(
...
): PagingSource<Int, FollowResponse>() {


    override fun getRefreshKey(state: PagingState<Int, FollowResponse>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, FollowResponse> {
        return try {
            ...
            val data = when(followType) {
                ...
                FollowType.MY_FOLLOWERS -> repository.getOtherUserFollowers(userId!!, page, params.loadSize)
                FollowType.MY_FOLLOWING -> repository.getOtherUserFollowing(userId!!, page, params.loadSize)
            }
